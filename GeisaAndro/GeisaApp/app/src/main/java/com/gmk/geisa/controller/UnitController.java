...
public class UnitController {
    ...
    private ArrayList<mUnit> userList = null;

    public static UnitController getInstance(Context context) {
        if (instance == null) {
    ...
    public void getAllUnitFromServer(final String salesId) {
        String cypertext;
        try {
        ...
        Call<ArrayList<mUnit>> userlist = service.getDataUnit(params);
        userlist.enqueue(new Callback<ArrayList<mUnit>>() {
            @Override
            public void onResponse(Call<ArrayList<mUnit>> call, Response<ArrayList<mUnit>> response) {
                //Log.e("isi respion cek update", response.code()+" isi code");
                if (response.isSuccessful()) {
                    //Log.e("isi respion", response.body().toString());
                    ArrayList<mUnit> result = response.body();
                    if (result != null) {
                        // Log.e("ada data", result.size()+" ada");
                        userList=result;
                        data.InsertUpateAllUnit(result);
            ...
            @Override
            public void onFailure(Call<ArrayList<mUnit>> call, Throwable t) {
    ...
    public  ArrayList<mUnit> getAllUnit(){
        return  data.getUnitAll();
    }

    public ArrayList<mUnit> getUnitByStatus(int statusId){
        return  data.getUnitByStatus(String.valueOf(statusId));
    }

    public mUnit getUnitByStatus(String unitId){
        return  data.getUnitById(unitId);
    }
